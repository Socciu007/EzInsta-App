...
const actionChatUser = async (page, directMsgObj, quantityChat) => {
    try {
        ...
        if (directMsgObj.UIDList.length > 0) {
            const quantityNeedChat = quantityChat < directMsgObj.UIDList.length ? quantityChat : directMsgObj.UIDList.length
            for (let i = 0; i < quantityNeedChat; i++) {
                let msgNewEle = await getElement(page, '[d="M10.002 17.226H6.774v-3.228L18.607 2.165a1.417 1.417 0 0 1 2.004 0l1.224 1.225a1.417 1.417 0 0 1 0 2.004Z"]')
                if (!msgNewEle) {
                    msgNewEle = await getElement(page, '[class="xsag5q8 x7sb2j6"] [class="x6s0dn4 x78zum5 xdt5ytf xl56j7k"]')
                    if (!msgNewEle) {
                        msgNewEle = await getElement(page, '[class="x9f619 x1n2onr6 x1ja2u2z x78zum5 xdt5ytf x2lah0s x193iq5w x6s0dn4 x1swvt13 x1pi30zi x1cnzs8"] [role="button"]')
                    }
                }
                await delay(getRandomIntBetween(3000, 5000))
                if (msgNewEle) {
                ...
                const closeEle = await getElement(page, '[points="20.643 3.357 12 12 3.353 20.647"]')
                await delay(getRandomIntBetween(3000, 5000))
                if (searchEle) {
                    await searchEle.type(directMsgObj.UIDList[i], { delay: 500 })
                    await delay(getRandomIntBetween(3000, 5000))
                    const selectEle = await getElement(page, '[name="ContactSearchResultCheckbox"]')
                    await delay(getRandomIntBetween(3000, 5000))
                    if (selectEle) {
                        await clickElement(selectEle)
                        await delay(getRandomIntBetween(3000, 5000))
                        const chatUserEle = await getElement(page, '[class="x9f619 xjbqb8w x78zum5 x168nmei x13lgxp2 x5pf9jr xo71vjh xw7yly9 xktsk01 x1yztbdb x1d52u69 x1uhb9sk x1plvlek xryxfnj x1c4vz4f x2lah0s xdt5ytf xqjyukv x1qjc9v5 x1oa3qoh x1nhvcw1"]')
                        if (chatUserEle) {
...
const actionChatWithFollowing = async (page, directMsgObj, indexOfUID) => {
    try {
        let arrIndex = [];
        if (directMsgObj.typeNew === 'random') {
            let followingEle = await getElements(page, '[class="_aarh"]')
            if (!followingEle) {
                followingEle = await getElements(page, '[class="_ap3a _aaco _aacw _aacx _aad7 _aade"]')
            }
            await delay(getRandomIntBetween(3000, 5000))
            if (followingEle.length > 0) {
                ...
                await delay(getRandomIntBetween(3000, 5000));
                let messageEle = await getElement(page, '[class="x1i10hfl xjqpnuy xa49m3k xqeqjp1 x2hbi6w x972fbf xcfux6l x1qhh985 xm0m39n xdl72j9 x2lah0s xe8uvvx xdj266r x11i5rnm xat24cr x1mh8g0r x2lwn1j xeuugli xexx8yu x18d9i69 x1hl2dhg xggy1nq x1ja2u2z x1t137rt x1q0g3np x1lku1pv x1a2a7pz x6s0dn4 xjyslct x1lq5wgf xgqcy7u x30kzoy x9jhf4c x1ejq31n xd10rxx x1sy0etr x17r0tee x9f619 x1ypdohk x78zum5 x1f6kntn xwhw2v2 x10w6t97 xl56j7k x17ydfre x1swvt13 x1pi30zi x1n2onr6 x2b8uid xlyipyv x87ps6o x14atkfc xcdnw81 x1i0vuye x1gjpkn9 x5n08af xsz8vos"]');
                if (!messageEle) {
                    messageEle = await getElement(page, '[class="x9f619 xjbqb8w x78zum5 x168nmei x13lgxp2 x5pf9jr xo71vjh x1i64zmx x1n2onr6 x6ikm8r x10wlt62 x1iyjqo2 x2lwn1j xeuugli xdt5ytf xqjyukv x1qjc9v5 x1oa3qoh x1nhvcw1"] [role="button"]')
                }
                if (messageEle) {
                    await delay(getRandomIntBetween(3000, 5000));
        ...
        if (directMsgObj.typeNew === 'user') {
            ...
            if (searchEle) {
                ...
                if (!followingEle) {
                    followingEle = await getElement(page, '[class="_ap3a _aaco _aacw _aacx _aad7 _aade"]')
                }
                await delay(getRandomIntBetween(3000, 5000))
                if (followingEle) {
...
const actionChat = async (page, directMsgObj) => {
    try {
        const contentMsg = directMsgObj.messageList[getRandomInt(directMsgObj.messageList.length)];
        ...
        if (importMsgEle) {
            ...
            await delay(getRandomIntBetween(3000, 5000))
            const sendEle = await getElement(page, '[class="x1i10hfl xjqpnuy xa49m3k xqeqjp1 x2hbi6w xdl72j9 x2lah0s xe8uvvx xdj266r xat24cr x1mh8g0r x2lwn1j xeuugli x1hl2dhg xggy1nq x1ja2u2z x1t137rt x1q0g3np x1lku1pv x1a2a7pz x6s0dn4 xjyslct x1ejq31n xd10rxx x1sy0etr x17r0tee x9f619 x1ypdohk x1f6kntn xwhw2v2 xl56j7k x17ydfre x2b8uid xlyipyv x87ps6o x14atkfc xcdnw81 x1i0vuye xjbqb8w xm3z3ea x1x8b98j x131883w x16mih1h x972fbf xcfux6l x1qhh985 xm0m39n xt0psk2 xt7dq6l xexx8yu x4uap5 x18d9i69 xkhd6sd x1n2onr6 x1n5bzlp x173jzuc x1yc6y37 xfs2ol5"]')
            if (sendEle) {
...
const selectorHref = async (page, namePage = "", indexHref) => {
  ...
  try {
    ...
    const quantityChat = getRandomIntBetween(directMsgObj.numberStart, directMsgObj.numberEnd);
    if (directMsgObj.typeDirectMSg === 'user') {
        ...
        if (directMsgObj.typeNew === 'random') {
            for (let i = 0; i < quantityChat; i++) {
                ...
                await delay(getRandomIntBetween(3000, 5000));
                if (isAccessProfile) {
                    await actionChatWithFollowing(page, directMsgObj)
                    await delay(getRandomIntBetween(3000, 5000))
                } else {
                    i--;
                    continue;
        ...
        logger("Debug|DirectMessage|You need import type of direct message. Ex: user, following,...");
        return;
    }
} catch (error) {
